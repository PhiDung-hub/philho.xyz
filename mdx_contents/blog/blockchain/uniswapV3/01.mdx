...
\Large{
  \lambda \overset{\Delta}= \frac {\delta L}{\delta \sqrt{P}} \\
}
$$
...
&\delta L \text{ - liquidity within the range, i.e. amount of token0 tradable in }[P, P + \delta P]

\end{aligned}
$$

Then liquidity curve is $\lambda - P$.
...
\bullet \ \delta L &= \Delta_{0 \rightarrow 1} Y \\
...
In fact, $\lambda_{i \rightarrow i+1} = L \ \forall i$, which mean liquidity is uniformly distributed
...
This issue is solved by concentrated liquidity in V3. A concentrated liquidity is, quoted, from V3-whitepaper:
"We call liquidity concentrated to a finite range a position. A position
...
<Image src="/blogs/blockchain/uniswapV3/01/virtual_reserve.webp" width="600" height="600" alt="virutal reserve" caption="Virtual reserve" />
<Image
...
So the new strategy is to split each tick into a virtual reserve then combine them to form the complete liquidity curve.
...
In the next installment, we'll dive into the practical implementation of this decentralized exchange.
Get ready for a deep exploration of its technical intricacies, cutting-edge technologies, and innovative mechanisms.
Stay tuned for Part 2, where theory meets reality. Cheers!
