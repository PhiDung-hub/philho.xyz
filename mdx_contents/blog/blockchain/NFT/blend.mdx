...
Instead, the following scheme is adapted (see [calcRefinancingAuctionRate()](https://github.com/PhiDung-hub/Blur_Clone/blob/main/src/BlurLending/CalculationHelpers.sol#L35)):
...
- `startAuction()` - lender can start the auction anytime by specifying the lien information and identifier
  ...
  - Note that the [liens](https://github.com/PhiDung-hub/Blur_Clone/blob/main/src/BlurLending/Blend.sol#L21) storage varialbe (`mapping(uint256 => bytes32)`) stores the actual lien hashing by an `uint256` key, hence lienId need to be specified
