...
\text{Given that }
& T \text{ - The auction duration, in seconds} \\
& R \text{ - Interest rate specified by old terms, in bips}
...
\begin{cases}
  ...
  t < \frac{4}{5} T, & 9,000 \cdot \frac{5t}{3T} + C_0 - 3,000 \\
  t < T, & 10,000 - \frac{100,000 - (C_0 + 9,000)}{t - 4T/5} \cdot t\\
  Else, & 100,000 &
\end{cases}
$$
...
- Black line is the **refinancing premium** if Debt value at the time of refinancing is 60,000 unit, e.g ETH.
  This show how much the lender would benefit (or loss) if the contracts were refinanced at time $t$.
