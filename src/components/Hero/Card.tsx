...
const calc = (x: number, y: number, rect: DOMRect, onlyImg: boolean) => [
  -(y - rect.top - rect.height / 2) / (onlyImg ? 10 : 24),
  (x - rect.left - rect.width / 2) / (onlyImg ? 10 : 24),
  onlyImg ? 1.10 : 1.01,
]
...
hover:after:opacity-100 hover:after:animate-hue-animation h-full z-20`}
          >
            {imgSrc && <NextImage alt={title} src={imgSrc} className="object-center" width={imgWidth} height={imgHeight} />}
            {onlyImg && <>{children}</>}
            {!onlyImg && (
              <div className="p-5">
